<link rel="stylesheet" href="{{ URL::asset('/resources/assets/admin/bootstrap/css/bootstrap.min.css')}}">
<!-- Font Awesome -->
<link rel="stylesheet" href="{{ URL::asset('/resources/assets/admin/bootstrap/css/font-awesome.css')}}">
<!-- Ionicons -->
<link rel="stylesheet" href="{{ url('https://cdnjs.cloudflare.com/ajax/libs/ionicons/2.0.1/css/ionicons.min.css')}}">
<!-- jvectormap -->
<link rel="stylesheet" href="{{ URL::asset('/resources/assets/admin/plugins/jvectormap/jquery-jvectormap-1.2.2.css')}}">
<!-- Theme style -->
<link rel="stylesheet" href="{{ URL::asset('/resources/assets/admin/dist/css/AdminLTE.min.css')}}">
<!-- AdminLTE Skins. Choose a skin from the css/skins
     folder instead of downloading all of them to reduce the load. -->
<link rel="stylesheet" href="{{ URL::asset('/resources/assets/admin/dist/css/skins/_all-skins.min.css')}}">
{{-- data table --}}
<link rel="stylesheet" href="{{ URL::asset('/resources/assets/admin/plugins/datatables/dataTables.bootstrap.css')}}">
<link rel="stylesheet" href="{{ URL::asset('/resources/assets/admin/plugins/select/bootstrap-select.css')}}">
<link rel="stylesheet" href="{{ URL::asset('/resources/assets/admin/plugins/datepicker/datepicker3.css')}}">

<link rel="stylesheet" href="{{ URL::asset('/resources/assets/admin/plugins/bootstrap-tagsinput-latest/bootstrap-tagsinput.css')}}">
<link rel="stylesheet" href="{{ URL::asset('/resources/assets/admin/plugins/pace/pace.css')}}">
{{--  toaster alert js  --}}
<link rel="stylesheet" href="{{ url('https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/toastr.min.css')}}">
<link rel="stylesheet" href="{{URL::asset('resources/assets/admin/plugins/select2/select2.min.css')}}">
<link rel="stylesheet" href="{{ URL::asset('/resources/assets/custom/developer.css')}}">

<script src="{{ URL::asset('/resources/assets/admin/plugins/jQuery/jquery-2.2.3.min.js')}}"></script>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datepicker/1.7.1/css/bootstrap-datepicker3.css">

<!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->
<!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
<!--[if lt IE 9]>
<script src="https://oss.maxcdn.com/html5shiv/3.7.3/html5shiv.min.js"></script>
<script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
<![endif]-->
